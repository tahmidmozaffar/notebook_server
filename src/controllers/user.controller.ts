        ...
        try {
          ...
          if (err instanceof Error) {
            return res.status(500).json({
              message: `Something went wrong. Please try again later. Exception: ${err.message}`,
            });
          }
        }
      } else {
...
const deleteProfile = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res
      .status(404)
      .send({ message: "Something went wrong. Could not find the user." });
  } catch (error) {
    if (error instanceof Error) {
      console.log(error.message);
    }
...
const resetPassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (user && user.id && user.email) {
      ...
      return res
        .status(200)
        .send({ message: "A verification code is sent to the account email." });
    } else {
      return res
        .status(200)
        .send({ message: "No user was found using this email" });
    }
  } catch (error) {
    if (error instanceof Error) {
      console.log(error.message);
...
const updatePassword = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!entry || entry.code !== parseInt(verificationCode)) {
      return res
        .status(401)
        .send({ message: "the verification code is not correct" });
    }
